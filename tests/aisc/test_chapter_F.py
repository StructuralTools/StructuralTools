...
def test_eq_F2_1():
    M_p = chapter_F.eq_F2_1(
        F_y=50*unit.ksi,
        Z_x=20*unit.inch**3,
        return_string=True)
    assert isclose(M_p, 1000/12*unit.kipft)
    assert M_p.value.units == "kipft"
    assert M_p.string == r"M_p &= F_y \cdot Z_x = 50\ \mathrm{ksi} \cdot 20\ \mathrm{in}^{3} &= 83.333\ \mathrm{kipft}"

def test_eq_F2_2():
    M_ltb = chapter_F.eq_F2_2(
        C_b=1,
        M_p=83*unit.kipft,
        F_y=50*unit.ksi,
        S_x=15*unit.inch**3,
    ...
    assert M_ltb.value.units == "kipft"
    assert M_ltb.string == r"""M_{ltb} &= C_b \cdot \left(M_p - \left(M_p - 0.7 \cdot F_y \cdot S_x\right) \cdot \left(\frac{L_b - L_p}{L_r - L_p}\right)\right)
    \\
...
def test_eq_F2_3():
    M_ltb = chapter_F.eq_F2_3(
        F_cr=17*unit.ksi,
        S_x=25.4*unit.inch**3,
        return_string=True)
    assert isclose(M_ltb, 35.98333333*unit.kipft, atol=1e-8*unit.kipft)
    assert M_ltb.value.units == "kipft"
    assert M_ltb.string == r"M_{ltb} &= F_{cr} \cdot S_x = 17\ \mathrm{ksi} \cdot 25.4\ \mathrm{in}^{3} &= 35.983\ \mathrm{kipft}"

def test_eq_F2_4():
    F_cr = chapter_F.eq_F2_4(
        C_b=1,
        E=29000*unit.ksi,
        L_b=15*unit.ft,
        r_ts=1.04*unit.inch,
    ...
    assert F_cr.value.units == "ksi"
    assert F_cr.string == r"""F_{cr} &= \frac{C_b \cdot \pi^2 \cdot E}{\left(\frac{L_b}{r_{ts}}\right)^2} \cdot \sqrt{1 + 0.078 \cdot \frac{J \cdot c}{S_x \cdot h_o} \cdot \left(\frac{L_b}{r_{ts}}\right)^2}
    \\
...
def test_eq_F2_5():
    L_p = chapter_F.eq_F2_5(
        r_y=0.848*unit.inch,
        E=29000*unit.ksi,
        F_y=50*unit.ksi,
        return_string=True)
    assert isclose(L_p, 2.995306513*unit.ft, atol=1e-8*unit.ft)
    assert L_p.value.units == "ft"
    assert L_p.string == r"L_p &= 1.76 \cdot r_y \cdot \sqrt{\frac{E}{F_y}} = 1.76 \cdot 0.848\ \mathrm{in} \cdot \sqrt{\frac{29000\ \mathrm{ksi}}{50\ \mathrm{ksi}}} &= 2.995\ \mathrm{ft}"

def test_eq_F2_6():
    L_r = chapter_F.eq_F2_6(
        r_ts=1.04*unit.inch,
        E=29000*unit.ksi,
        F_y=50*unit.ksi,
        J=0.293*unit.inch**4,
    ...
    assert L_r.value.units == "ft"
    assert L_r.string == r"""L_r &= 1.95 \cdot r_{ts} \cdot \frac{E}{0.7 \cdot F_y} \cdot \sqrt{\frac{J \cdot c}{S_x \cdot h_o} + \sqrt{\left(\frac{J \cdot c}{S_x \cdot h_o}\right)^2 + 6.76 \cdot \left(\frac{0.7 \cdot F_y}{E}\right)^2}}
    \\
...
def test_sec_F2_1():
    ...
    M_p = chapter_F.sec_F2_1(shape, return_string=True)
...
def test_eq_F3_1():
    ...
    assert M_flb.value.units == "kipft"
    assert M_flb.string == r"""M_{flb} &= M_p - \left(M_p - 0.7 \cdot F_y \cdot S_x\right) \left(\frac{\lambda_f - \lambda_{pf}}{\lambda_{rf} - \lambda_{pf}}\right)
    \\
...
def test_eq_F3_2():
    ...
    assert M_flb.value.units == "kipft"
    assert M_flb.string == r"M_{flb} &= \frac{0.9 \cdot E \cdot k_c \cdot S_x}{\lambda_f^2} = \frac{0.9 \cdot 29000\ \mathrm{ksi} \cdot 0.5 \cdot 25\ \mathrm{in}^{3}}{30^2} &= 30.208\ \mathrm{kipft}"

def test_eq_F3_2a_low():
    k_c = chapter_F.eq_F3_2a(lamb_w=250, return_string=True)
    assert k_c == 0.35
    assert k_c.string == r"k_c &= \operatorname{min}\left(\operatorname{max}\left(0.35,\ \frac{4}{\sqrt{\lambda_w}}\right),\ 0.76\right) = \operatorname{min}\left(\operatorname{max}\left(0.35,\ \frac{4}{\sqrt{250}}\right),\ 0.76\right) &= 0.35"

def test_eq_F3_2a_calculated():
    k_c = chapter_F.eq_F3_2a(lamb_w=100, return_string=True)
    assert isclose(k_c, 0.4)
    assert k_c.string == r"k_c &= \operatorname{min}\left(\operatorname{max}\left(0.35,\ \frac{4}{\sqrt{\lambda_w}}\right),\ 0.76\right) = \operatorname{min}\left(\operatorname{max}\left(0.35,\ \frac{4}{\sqrt{100}}\right),\ 0.76\right) &= 0.4"

def test_eq_F3_2a_high():
    k_c = chapter_F.eq_F3_2a(lamb_w=20, return_string=True)
    assert k_c == 0.76
    assert k_c.string == r"k_c &= \operatorname{min}\left(\operatorname{max}\left(0.35,\ \frac{4}{\sqrt{\lambda_w}}\right),\ 0.76\right) = \operatorname{min}\left(\operatorname{max}\left(0.35,\ \frac{4}{\sqrt{20}}\right),\ 0.76\right) &= 0.76"

def test_sec_F3_2_noncompact():
    shape = aisc.WideFlange("W10X12", materials.Steel("A992"))
    M_flb = chapter_F.sec_F3_2(shape, 52.5*unit.kipft, return_string=True)
    assert isclose(M_flb, 52.11390857*unit.kipft, atol=1e-8*unit.kipft)
    assert M_flb.string == r"""\begin{aligned}
...
def test_sec_F3():
    shape = aisc.WideFlange("W10X12", materials.Steel("A992"))
    M_n = chapter_F.sec_F3(shape, 0*unit.ft, 1, return_string=True)
